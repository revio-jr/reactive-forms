...
export const useForm = <T extends string, TData extends object>({
    ...
    const handleInputChange = useCallback((e: Event) => {
        ...
        reactiveForm.current.setDirty({ [name]: true } as Record<keyof TData, boolean>);
    ...
    const handleFocusChange = useCallback((e: Event) => {
        ...
        reactiveForm.current.setTouched({ [name]: true } as Record<keyof TData, boolean>);
    }, [reactiveForm]);
    ...
    useEffect(() => {
        ...
        inputs.forEach(input => {
            ...
            input.value = String(reactiveForm.current!.defaultValue[input.name as keyof TData]);
        });
        ...
        selects.forEach(select => {
            ...
            select.value = String(reactiveForm.current!.defaultValue[select.name as keyof TData]);
        });
        ...
        textareas.forEach(textarea => {
            ...
            textarea.value = String(reactiveForm.current!.defaultValue[textarea.name as keyof TData]);
        });
        ...
        return () => {
            if (formRef.current) {
                ...
                const scriptEl = document.querySelector(`script[data-form-values="${formName}"]`);
                if (scriptEl) {
